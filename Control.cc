...
{
	Hotel* hp;
	resMgr = new ResManager(hp);
	hotel = new Hotel("My hotel", resMgr);
	resMgr->setHotel(hotel);
	GuestRecorder*	record1 = new GuestRecorder("GuestRecorder");
	StayRecorder*	record2 = new StayRecorder("StayRecorder");
	UpgradeRecorder* record3 = new UpgradeRecorder("UpgradeRecorder");
	resMgr->subscriber(record1);
	resMgr->subscriber(record2);
	resMgr->subscriber(record3);
}
